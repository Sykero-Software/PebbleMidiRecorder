...
def patch_clay_for_new_platforms(ctx):
    ...
    *package*, waf links that C lib into every target platform, so a build that
    ...
    matching include stubs. The bytes are never executed, so using another
    board's lib only needs to satisfy the linker. Idempotent; patches dist.zip
    (the build re-extracts it each time).
    """
